...
describe("TripSelector", () => {
  ...
  describe("Trip Selection", () => {
    ...
    it("calls onCreateTrip when Create New Trip is clicked", async () => {
      const user = userEvent.setup();
      const onCreateTrip = jest.fn();
      render(<TripSelector {...defaultProps} onCreateTrip={onCreateTrip} />);
  ...
  describe("Keyboard Navigation", () => {
    ...
    it("selects trip with Enter key", async () => {
      const user = userEvent.setup();
      const onTripSelect = jest.fn();
      render(<TripSelector {...defaultProps} onTripSelect={onTripSelect} />);

      // Open dropdown and navigate to first trip
      const trigger = screen.getByRole("button", { name: /select a trip/i });
      await user.click(trigger);
      await user.keyboard("{ArrowDown}"); // Focus "Create New Trip"
      await user.keyboard("{ArrowDown}"); // Focus first trip
      await user.keyboard("{Enter}"); // Select first trip

      expect(onTripSelect).toHaveBeenCalledWith("trip-1");
    });

    it("creates trip with Enter key on Create New Trip option", async () => {
      const user = userEvent.setup();
      const onCreateTrip = jest.fn();
      render(<TripSelector {...defaultProps} onCreateTrip={onCreateTrip} />);

      // Open dropdown and select Create New Trip
      const trigger = screen.getByRole("button", { name: /select a trip/i });
      await user.click(trigger);
      await user.keyboard("{ArrowDown}"); // Focus "Create New Trip"
      await user.keyboard("{Enter}"); // Select Create New Trip
