...
import type { Activity, ItineraryData, ItineraryDay, TimeOfDay } from "../types/itinerary";
...
export function useItineraryDeletion(
    ...
    (
      ...
      for (const tod of TIME_OF_DAY_ORDER) {
        ...
        activities.forEach((activity, idx) => {
          const id = `${tod}-${idx}-${activity.title}`;
      ...
      // Build a set of activity titles to remove (keyed by timeOfDay + title)
      const removeByTitle = new Set(
        removed.map((r) => `${r.timeOfDay}::${r.activity.title}`),
      );
      ...
      setLocalDays((prev) => {
        return prev.map((day) => {
          ...
          const newActivities = day.activities.filter((a) => {
            ...
            return !removeByTitle.has(`${tod}::${a.title}`);
          });
  ...
  const undo = useCallback(() => {
    setDeletionStack((prev) => {
      ...
      setLocalDays((days) =>
        days.map((day) => {
          if (day.day_number !== entry.dayNumber) return day;
    ...
    async (itineraryRowId: string) => {
      ...
      try {
        ...
        const { error } = await supabase
          ...
          .update({
            itinerary: modifiedItinerary as unknown as any,
  ...
  const hasUnsavedChanges = useMemo(() => {
    return JSON.stringify(localDays) !== JSON.stringify(savedSnapshot);
