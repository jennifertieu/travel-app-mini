      ...
      annotations.forEach((ann) => {
        ...
        if (coords.type === "polygon" && Array.isArray(coords.points)) {
          ...
          if (ann.label || (ann as any).name) {
            const avgLat =
              latLngs.reduce((sum: number, p: [number, number]) => sum + p[0], 0) / latLngs.length;
            const avgLng =
              latLngs.reduce((sum: number, p: [number, number]) => sum + p[1], 0) / latLngs.length;
          ...
          try {
            if (data.intent === "search_area") {
              ...
              if (error) {
                ...
                addAnnotationOptimistic(inserted as unknown as Annotation);
                onAnnotationSaved?.(inserted as unknown as Annotation);
              }
            }
          } catch (error) {
