...
// Helper that mirrors the component's formatDate logic so tests aren't
// brittle against timezone-dependent date rendering.
function formatDate(
  dateString: string,
  ...
  return date.toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: date.getFullYear() !== referenceYear ? "numeric" : undefined,
  });
}
...
describe("TripItem", () => {
  ...
  describe("Trip Information Display", () => {
    ...
    it("should display formatted date range when both dates are available", () => {
      ...
      const expectedStart = formatDate("2024-03-01");
      const expectedEnd = formatDate("2024-03-10");
      expect(
    ...
    it("should display start date only when end date is missing", () => {
      ...
      const expectedStart = formatDate("2024-03-01");
      expect(screen.getByText(`From ${expectedStart}`)).toBeInTheDocument();
    });
    ...
    it("should display end date only when start date is missing", () => {
      ...
      const expectedEnd = formatDate("2024-03-10");
      expect(screen.getByText(`Until ${expectedEnd}`)).toBeInTheDocument();
    });
    ...
    it("should not display date range when both dates are missing", () => {
      ...
      // No date text should appear (look for absence of month abbreviations)
      const dateDiv = screen.queryByText(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)/);
    ...
    it("should include year in date format when different from current year", () => {
      ...
      const expectedStart = formatDate("2000-03-01");
      const expectedEnd = formatDate("2000-03-10");
      expect(
        screen.getByText(`${expectedStart} - ${expectedEnd}`),
      ).toBeInTheDocument();
    });
  ...
  describe("Selection Highlighting", () => {
    it("should show check icon when trip is selected", () => {
      const { container } = render(<TripItem {...defaultProps} isSelected={true} />);

      // Lucide icons render as aria-hidden SVGs; query the DOM directly
    ...
    it("should not show check icon when trip is not selected", () => {
      const { container } = render(<TripItem {...defaultProps} isSelected={false} />);
  ...
  describe("Accessibility", () => {
    ...
    it("should have descriptive aria-label", () => {
      ...
      expect(button).toHaveAttribute(
        "aria-label",
        `Japan Adventure, ${expectedStart} - ${expectedEnd}`,
      );
    });
    ...
    it("should include selection status in aria-label when selected", () => {
      ...
      expect(button).toHaveAttribute(
        "aria-label",
        `Japan Adventure, ${expectedStart} - ${expectedEnd} (currently selected)`,
      );
    });
    ...
    it("should mark check icon as decorative", () => {
      ...
      // Lucide icons render as aria-hidden SVGs
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle invalid date formats gracefully", () => {
      ...
      const { container } = render(
        <TripItem {...defaultProps} trip={invalidDateTrip} />,
      );

      // Component should render without crashing; avoid asserting on
      // engine-specific "Invalid Date" strings that differ across environments.
