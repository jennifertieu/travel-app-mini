...
export function AddIdeaModal() {
  const { isOpen, closeModal, getModalData } = useModal();
  ...
  const { startStreaming, isStreaming } = (getModalData("addIdea") ?? {}) as { startStreaming?: (args: any) => void; isStreaming?: boolean };
  ...
  const startEnrichmentPipeline = async (videoUrl: string) => {
    if (!currentTripId || !member || !currentTrip) {
